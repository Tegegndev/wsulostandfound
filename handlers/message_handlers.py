...
def register_message_handlers(bot):
    ...
    def handle_button_press(message: telebot.types.Message):
        try:
            ...
            logger.exception("handle_button_press failed")
            report_to_admin(bot, "handle_button_press", e)
        ...
        try:
            ...
            if state.get("kind") == "registration" and state.get("step") == "phone":
                ...
                try:
                    ...
                    report_to_admin(bot, "handle_contact.create_user", e)
                    bot.send_message(chat_id, "Error during registration. Please try again later.")
                clear_user_state(chat_id)
        except Exception as e:
            logger.exception("handle_contact failed")
            report_to_admin(bot, "handle_contact", e)
    ...
    def handle_post_flow(message: telebot.types.Message):
        try:
            ...
            logger.exception("handle_post_flow failed")
            report_to_admin(bot, "handle_post_flow", e)
            bot.send_message(message.chat.id, "An error occurred. Please try again later.")
